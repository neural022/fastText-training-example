...
class FastText {
 ...
 public:
  ...
  void predict(std::istream&, int32_t, std::vector<std::pair<real, std::string>>&, real = 0.0) const;
  void ngramVectors(std::string);
  void precomputeWordVectors(Matrix&);
  void findNN(const Matrix&, const Vector&, int32_t, const std::set<std::string>&, std::vector<std::pair<real, std::string>>& results);
