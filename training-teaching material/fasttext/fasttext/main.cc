...
void test(const std::vector<std::string>& args) 
{
	...
	if (infile == "-") result = fasttext.test(std::cin, k, threshold);
	else 
	{
...
void predict(const std::vector<std::string>& args) 
{
	...
	std::string infile(args[3]);
	if (infile == "-") fasttext.predict(std::cin, k, print_prob, threshold);
	else 
	{
		std::ifstream ifs(infile);
		if (!ifs.is_open()) 
...
void nn(const std::vector<std::string> args) 
{
	int32_t k;
	if (args.size() == 3) k = 10;
	else if (args.size() == 4) k = std::stoi(args[3]);
	else 
	{
		printNNUsage();
		exit(EXIT_FAILURE);
...
void train(const std::vector<std::string> args) 
{
	...
	if (!ofs.is_open()) throw std::invalid_argument(a.output + ".bin cannot be opened for saving.");
	ofs.close();
	fasttext.train(a);
	fasttext.saveModel();
	fasttext.saveVectors();
	if (a.saveOutput) fasttext.saveOutput();
}
...
void dump(const std::vector<std::string>& args) 
{
	...
	if (option == "args") fasttext.getArgs().dump(std::cout);
	else if (option == "dict") fasttext.getDictionary()->dump(std::cout);
	else if (option == "input") 
	{
		if (fasttext.isQuant()) std::cerr << "Not supported for quantized models." << std::endl;
		else fasttext.getInputMatrix()->dump(std::cout);
	}
	else if (option == "output") 
	{
		if (fasttext.isQuant()) std::cerr << "Not supported for quantized models." << std::endl;
		else fasttext.getOutputMatrix()->dump(std::cout);
	}
	else 
	{
		printDumpUsage();
...
int main(int argc, char** argv) 
{
	...
	else if (command == "test") test(args);
	...
	else if (command == "predict" || command == "predict-prob") predict(args);
